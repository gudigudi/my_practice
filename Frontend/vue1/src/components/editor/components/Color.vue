<template>
  <div id="color">
    <a href="javascript:;" :title="compName==='foreColor' ? lang.foreColor : lang.backColor"
       :class="{'active': this.display,'disabled':this.disabled}" @click="toggle">
      <i :class="{'icon-file-text': compName==='backColor','icon-file-text-o':compName==='foreColor'}"></i>
    </a>
    <div class="toolbar-dropdown color-picker" v-show="display">
      <div class="input-box">
        <input type="text" class="input" :placeholder="lang.colorCode" v-model="color">
        <button type="button" class="btn" @click="inputHandler">{{lang.ok}}</button>
      </div>
      <ul>
