      ...
      <div :class="addressHeader">
        <ul>
          ...
          <template v-if="!onlyProvince">
            <li v-if="showCityTab"
                :class="{'active': tab == 2}"
                @click="resetCity">{{currentCity && !staticPlaceholder ? currentCity : placeholders.city}}
            </li>
            <li v-if="showAreaTab && !hideArea"
                :class="{'active': tab == 3}">{{currentArea && !staticPlaceholder ? currentArea : placeholders.area}}
            </li>
          </template>
        </ul>
      </div>
...
<script>
  import {DISTRICTS} from '../moke/districts'
  ...
  export default {
    ...
    methods: {
      ...
      getAreas () {
        this.currentArea = this.placeholders.area
        this.areas = this.determineValue(this.currentCity, this.placeholder.city)
        if (this.areas == null) {
          this.emit('selected')
          this.tab = 2
    ...
    watch: {
      ...
      province (value) {
        this.currentProvince = this.province || this.pl.province
        this.cities = this.determineValue(this.currentProvince, this.placeholders.province)
      },
      city (value) {
