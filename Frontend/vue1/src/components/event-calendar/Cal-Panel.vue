<template>
  <div class="cal-wrapper">
    ...
    <div class="cal-body">
      <div class="weeks">
        <span v-for="dayName in dayNames" class="item">{{dayName}}</span>
      </div>
      <div class="dates">
        <div v-for="date in dayList" class="item" :class="{
          today: date.status ? (today==date.date) : false,
          event: date.status ? (date.title!=undefined) : false,
          [calender.options.className] : (date.date == selectedDay)
        }">
          <p class="date-num" @click="handleChangeCurday(date)"
            ...
            {{date.statuc ? date.date.split('/')[2] : '&nbsp'}}
          </p>
          <span v-if="date.status ? (today === date.date) : false" class="is-today"
                :style="{backgruondColor:customColor}"></span>
...
<script>
  export default {
    ...
    computed: {
      ...
      curYearMonth () {
        let tempData = Date.parse(new Date(`${this.calendar.params.curYear}/${this.calendar.params.curMonth + 1}/01`))
        return this.vm.$moment(data, 'dd/MM/yyyy')
      },
      customColor () {
