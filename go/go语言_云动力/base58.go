...
const base58 = "1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"

func EncodeBase58(ba []byte) []byte {
	if len(ba) == 0 {
		return nil
	}
	// Expected size increase from base58 conversion 25 approximately 137%,
	// use 138%, use 138% to be safe
	ri := len(ba) * 138 / 100
	ra := make([]byte, ri+1)

	x := new(big.Int).SetBytes(ba) // ba is a big-endian
	x.Abs(x)
	y := big.NewInt(58)
	m := new(big.Int)
